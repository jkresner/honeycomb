module.exports = {


  tst1: {
    _id : ObjectId("54551be15f221efa17111111"),
    name : "Expert One",
    emails: [
      { _id:ObjectId("56131305a094059aa7070c6c"), value: "airpairtest1@gmail.com", verified: true, primary: true, origin: 'manual:input' }
    ],
    photos: [{ value: "6651e9e7b00694699b5f57968251626c", type: "gravatar", primary: true }],
    auth: {
      gh: {
        login: 'airpairtest1',
        id: 11261012,
        avatar_url: 'https://avatars.githxubusercontent.com/u/11261012?v=3',
        gravatar_id: '',
        name: "Onesy",
        emails: [{ email:"test1ap@gmail.com",primary:true,verified:true}],
        company: null,
        blog: null,
        location: null,
        email: null,
        hireable: null,
        bio: null,
        public_repos: 2,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2015-03-01T21:12:29Z',
        updated_at: '2015-09-07T07:11:18Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        disk_usage: 0,
        collaborators: 0,
        plan:
         { name: 'free',
           space: 976562499,
           collaborators: 0,
           private_repos: 0 },
        tokens: { "auth-ex1": { token: 'aa1905fec86e484072fd210acfd27537c65faefa' } }
      }
    },
    "log": { "history" : [] }
  },


  tst5: {
    _id : ObjectId("54551be15f221efa17111115"),
    name : "Expert Five",
    emails: [{ _id:ObjectId("56131305a094059aa7070c6d"), value: "airpairtest5@gmail.com", verified: true, primary: true, origin: 'oauth:github' }],
    auth: {
      gh: {
        login: 'airpairtest5',
        id: 11262470,
        avatar_url: 'https://avatars.githubusercontent.com/u/11262470?v=3',
        gravatar_id: '5a086460c6db9145493b9192faa4b0bc',
        name: "Fiver",
        company: null,
        blog: null,
        location: null,
        emails: [{ email:"airpairtest5@gmail.com",primary:true,verified:true}],
        hireable: null,
        bio: null,
        public_repos: 182,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2015-03-02T00:14:27Z',
        updated_at: '2015-09-07T07:16:41Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        disk_usage: 0,
        collaborators: 0,
        plan:
         { name: 'free',
           space: 976562499,
           collaborators: 0,
           private_repos: 0 },
        tokens: { "auth-ex1": { token: '261d9670b74fed23b2098a55d9bccf55a7526661' } }
      }
    },
    "log": { "history" : [] }
  },


  tst11: {
    _id : ObjectId("541111111111111a11111111"),
    name : "Expert Elevan",
    emails: [{ _id:ObjectId("56131305a094059aa7070991"), value: "airpairtest11@gmail.com", verified: true, primary: true, origin: 'manual:input' }],
    auth: {
      gh: {
        login: 'airpairtest11',
        id: 11111111,
        avatar_url: 'https://avatars.githxubusercontent.com/u/11111111?v=3',
        gravatar_id: '',
        name: "Elevens",
        emails: [{ email:"test11ap@gmail.com",primary:true,verified:true}],
        company: 'Unset co.',
        blog: null,
        location: null,
        email: null,
        hireable: null,
        bio: null,
        public_repos: 2,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2015-03-01T21:12:29Z',
        updated_at: '2015-09-07T07:11:18Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        disk_usage: 0,
        collaborators: 0,
        plan:{},
        tokens: { "auth-ex1": { token: '1111' } }
      },
      so: {
        "badge_counts":{"bronze":11,"silver":11,"gold":11},
        "account_id":7111111,
        "last_access_date":1111111109,
        "reputation_change_year":11,
        "reputation_change_quarter":11,
        "reputation_change_month":11,
        "reputation_change_week":11,
        "reputation_change_day":11,
        "reputation":11,
        "creation_date":1447758209,
        "user_type":"registered",
        "user_id":1171111,
        "link":"http://stackoverflow.com/users/1171111/air-pair",
        "profile_image":"https://lh6.googleusercontent.com/-tEGuZ5EdOw0/AAAAAAAAAAI/AAAAAAAAAEo/Tvt0ZoRCwv0/photo.jpg?sz=128",
        "display_name":"Air Eleven"
      },
      in: {
        "lastName" : "Eleven",
        "id" : "d9YF1111",
        "firstName" : "Airey"
      }
    },
    "log": { "history" : [] }
  },


  jkg: {
    "_id" : ObjectId("549342348f8c80299bcc56c2"),
    "emails" : [ { _id:ObjectId("56131305a094059aa7070c6e"), value: "jkresner@gmail.com", verified: true, primary: true } ],
    "name" : "Jonathon Kresner",
    "auth" : {
      "gh" : {
        "login" : "jkresner",
        "id" : 979542,
        "avatar_url" : "https://avatars.githubusercontent.com/u/979542?v=3",
        "gravatar_id" : "",
        "name" : "Jonathon Kresner",
        "company" : "airpair, inc.",
        "blog" : "http://www.hackerpreneurialism.com",
        "location" : "San Francisco",
        "emails": [{email:"jkresner@gmail.com",primary:true,verified:true}],
        "hireable" : false,
        "bio" : null,
        "public_repos" : 1,
        "public_gists" : 1,
        "followers" : 401,
        "following" : 1,
        "created_at" : "2011-08-14T17:14:37Z",
        "updated_at" : "2015-02-20T19:36:09Z",
        "private_gists" : 1,
        "total_private_repos" : 1,
        "owned_private_repos" : 1,
        "disk_usage" : 102347,
        "collaborators" : 1,
        "plan" : {
          "name" : "micro",
          "space" : 614400,
          "collaborators" : 0,
          "private_repos" : 0
        },
        tokens: { "auth-ex1": { token: 'bbb90666686e484072fd210acfd27537c65faefa' } }
      }
    },
    "username" : "jkres",
    "log": { "history" : [] }
  },


  jky: {
    "_id" : ObjectId("549342348f8c80333cccc6c2"),
    "emails" : [ { _id:ObjectId("56131305a094059aa7070c61"), value: "jkresner@yahoo.com.au", verified: true, primary: true, origin: 'manual:input' } ],
    "photos": [{ value: "2aefff5ada0d266491e9fd7a15b91941", type: "gravatar", primary: true }],
    "name" : "Jonathon Yahoo",
    "auth" : {
      "password": { hash: "$2a$08$Dh7lJhGKpct93rGQvjXrJeChI5yh4XctZolRqCrtZ.xkZH5DpZY6C" },
      "gh": {
        login: 'jkyahoo',
        id: 11258947,
        avatar_url: 'https://avatars.githubusercontent.com/u/11258947?v=3',
        gravatar_id: '',
        name: "Jonathon Yahoo",
        emails: [{email:"jkresner@yahoo.com.au",primary:false,verified:true}],
        company: null,
        blog: null,
        location: null,
        email: null,
        hireable: null,
        bio: null,
        public_repos: 1,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2015-03-01T17:30:48Z',
        updated_at: '2015-10-05T16:05:32Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        disk_usage: 0,
        collaborators: 0,
        plan: { name: 'free', space: 976562499, collaborators: 0, private_repos: 0 },
        tokens: { "auth-ex1": { token: '34f4c2e52eaf0134a669a771255668a02dacd122' } }
      }
    },
    "username" : "jkresnerau",
    "log": {
      "history" : [ { "_id" : ObjectId("5612a3cbb533e44fa42ce276"),
                      "action" : "signup",
             "by" : { "_id" : ObjectId("549342348f8c80333cccc6c2"), "name" : "Jonathon Yahoo" } }],
      "last" : {
        "_id" : ObjectId("5612a3cbb533e44fa42ce276"),
        "action" : "signup",
        "by" : { "_id" : ObjectId("549342348f8c80333cccc6c2"), "name" : "Jonathon Yahoo" }
      }
    }
  },

  jkc: {
    "_id" : ObjectId("597e2f52755541797ea3d8b7"),
    "name" : "J Koala",
    "auth" : {
        "fb" : {
            "id" : "218300862",
            "name" : "Jonathon Kresner",
            "last_name" : "Kresner",
            "first_name" : "Jonathon",
            "gender" : "male",
            "link" : "https://www.facebook.com/app_scoped_user_id/218300862/",
            "email" : "jk@climbfind.com",
            "picture" : { "data" : {
                    "width" : 480,
                    "url" : "https://scontent.xx.fbcdn.net/v/t31.0-1/c124.0.480.480/p480x480/1292368_639194289544_253203027_o.jpg",
                    "is_silhouette" : false,
                    "height" : 480
                }
            },
            "tokens" : {}
        }
    },
    "log" : {
        "comm" : { "welcome" : ObjectId("59895f94b0d625000454f857") },
        "last" : {
            "by" : {},
            "_id" : ObjectId("5a13a05de6bb190004742f86"),
            "action" : "login"
        },
        "history" : []
    },
    "photos" : [ 
        {
            "value" : "https://scontent.xx.fbcdn.net/v/t31.0-1/c124.0.480.480/p480x480/1292368_639194289544_253203027_o.jpg",
            "type" : "facebook",
            "primary" : true,
            "_id" : ObjectId("597e2f52755541797ea3d8b9")
        },
    ],
    "emails" : [{
      "_id" : ObjectId("597e2f52755541797ea3d8b6"),
      "primary" : true,
      "value" : "jk@climbfind.com",
      "verified" : true,
      "origin" : "oauth:facebook" }
    ],
    "settings" : {
        "notify" : {},
        "tz" : { "utc_offset" : 420, "id" : "America/Los_Angeles" }
    }
  },

  mwauthd: {
    "_id" : ObjectId("598962e3735a750004278ffe"),
    "name" : "Jaye Kaye",
    "auth" : {
      "gh": {
        login: 'jkcf',
        id: 13332947,
        avatar_url: 'https://avatars.githubusercontent.com/u/11258947?v=3',
        gravatar_id: '',
        name: "Jaye Kaye",
        emails: [{email:"jk@cf.com",primary:false,verified:true}],
        company: null,
        blog: null,
        location: null,
        email: null,
        hireable: null,
        bio: null,
        public_repos: 1,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2012-03-01T17:30:48Z',
        updated_at: '2012-10-05T16:05:32Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        disk_usage: 0,
        collaborators: 0,
        plan: { name: 'free', space: 976562499, collaborators: 0, private_repos: 0 },
        tokens: { "auth-ex1": { token: 'tokkkk' } }
      }
    },
    "log" : { "history" : [] },
    "photos" : [{ "_id" : ObjectId("598962e3735a750004279000"), "primary" : true, "type" : "facebook", "value" : "https://scontent.xx.fbcdn.net/v/t1.0-1/20604167_103941463641957_2113239479145543037_n.jpg?oh=dff390d48e8708f836eb74ed48d8f5d6&oe=5A2F7F71" }],
    "emails" : [{ "origin" : "oauth:github", "verified" : true, "value" : "jk@cf.com", "primary" : true, "_id" : ObjectId("598962e3735a750004278ffd") }]
  },

  auth_unverified: {
    "auth" : {
      "gh": {
        login: 'jokaye',
        id: 11112127,
        avatar_url: 'https://avatars.githubusercontent.com/u/11258947?v=3',
        name: "Jono Kaye",
        emails: [{email:"jok@test.com",primary:false,verified:false}],
        public_repos: 1,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2012-03-01T17:30:48Z',
        updated_at: '2012-10-05T16:05:32Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        tokens: { "auth-ex1": { token: 'tokkeekk' } }
      }
    }
  },

  auth_ghnew: {
    "auth" : {
      "gh": {
        login: 'jokaye',
        id: 11112127,
        avatar_url: 'https://avatars.githubusercontent.com/u/11258947?v=3',
        name: "Jono Kaye",
        emails: [
          { email:"jok@test.com",primary:true,verified:true },
          { email:"jok@tst.co",primary:false,verified:true }
        ],
        public_repos: 1,
        public_gists: 0,
        followers: 0,
        following: 0,
        created_at: '2012-03-01T17:30:48Z',
        updated_at: '2012-10-05T16:05:32Z',
        private_gists: 0,
        total_private_repos: 0,
        owned_private_repos: 0,
        tokens: { "auth-ex1": { token: 'tokkeekk' } }
      }
    }
  }



}
