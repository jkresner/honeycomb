...
module.exports = ({User}, Data) => ({
  ...
  linkOAuth({user,existing}, key, provider, profile, tokens, done) {
    LOG('auth.link', `oauth:${key}:link`, user.name, JSON.stringify(profile).gray)
  ...
  loginLocal({sessionID, session}, existing, done) {
    LOG('auth.login', `pwd:login`, existing.name, existing._id)
  ...
  loginOAuth({sessionID, session, existing}, key, provider, profile, tokens, done) {
    ...
    LOG('auth.login', `oauth:${key}:login`, profile.displayName||profile.name||profile.id)
  ...
  signup(sessionID, session, signup, done) {
    ...
    signup._id = User.newId()  // So touch works
...
// }
  ...
  signupOAuth({sessionID,session}, key, provider, profile, {token,refresh}, done) {
    var {Project} = Data.auth
    LOG('auth.create', `oauth.${key}.signup`, profile.displayName||profile.name||`${profile.id} ${profile.login||profile.username}`)
