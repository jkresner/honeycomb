function logAct(on, action, user) {
  if (!on)
    on = { log: { history: [] } }
  else if (!on.log)
...
module.exports = ({User}, Data) => ({
  ...
  touchMeta(meta, action, user) {
    throw Error(`touchMeta ${action} deprecated. Please use DRY.logAct ${user.name}`)
  },
  ...
  loginLocal(ctx, existing, done) {
    ...
    this.login('local.pwd', sessionID, session, existing, updates, (e, login) => {
      done(e, login)
