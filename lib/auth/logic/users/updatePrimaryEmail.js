module.exports = function({User}, Data, DRY) {
  ...
  return {
    ...
    exec(email, done) {
      ...
      User.getById(this.user._id, '_id emails meta', (e, user) => {
        var {_id,emails,meta} = user
        for (var o of emails) o.primary = _.idsEqual(email._id,o._id)

        meta = DRY.touchMeta(meta,'update:primaryEmail',user)

        User.updateSet(_id, {emails,meta}, done)
      })
