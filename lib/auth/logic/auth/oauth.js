module.exports = function(DAL, Data, DRY) {
  ...
  function validate(user, existing, provider, profile, tokens) {
      ...
      if (provider == 'facebook')  {
        ...
        if (!profile.picture || !profile.picture.data ||
            profile.picture.data.is_silhouette) return `Facebook profile picture required`
      }
  ...
  return {
    ...
    exec(provider, profile, tokens, done) {
      ...
      var {short} = cfg[provider]
      DRY.auth.userByAuth(this.user, short, profile, (e, existing) => {
        if (e) return done(e)
        ...
        DRY.auth[fn](this, short, provider, profile, tokens, done)
      })
    },
