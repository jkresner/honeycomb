...
/                                                                           )*/
module.exports = function(cfgDir, env="dev", ops={dotEnv:true}) {

  // Allow passing config object as first param instead of dir of app.json
  ...
  //-- (not for production - rely only on process.env in production)
  if (ops.dotEnv && env != 'production')
  ...
  if (app.auth) 
    app.auth = Object.assign(require('../auth/config.defaults.js')(), app.auth)
  ...
  if (cfg.http.static) {
    let {favicon,bundles,host,manifest} = cfg.http.static
    if (favicon) cfg.http.static.favicon.root = join(appDir, favicon.root)

    dirs(cfg.http.static, '/')
    //-- review
    cfg.http.static.dirs = cfg.http.static.dirs || []

    let rev = manifest ? require(join(appDir, manifest)) : {}
    for (let bundle in rev)
