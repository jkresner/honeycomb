  ...
  _send(mail, opts, cb)
  {
    ...
    this.api.sendMail(mail, (e, info) => {
      if (!mail.from.match('ERR')) {
        LOG('comm.send', `smtp.to ${mail.from}`, mail.subject)
...
var sesWrapper = {
  ...
  init(config) {
    ...
    if (comm.mode == 'stub')
      ...
      // this.api = createTransport({ SES: new aws.SES({apiVersion: '2010-12-01'}) })
    }
  ...
  _send(mail, opts, cb)
  {
    ...
    this.api.sendMail(mail, (e, info) => {      
      // info.messageId, 
      LOG('comm.send', `SES send`, mail.subject, '>>'.dim, mail.to.join(','))
