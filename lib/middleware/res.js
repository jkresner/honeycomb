module.exports = ({$mw,$req},Lib) => ({
  ...
  *                                                                          */
  forbid(name, forbidden, opts) {
    ...
    return function(req, res, done) {
      ...
      if (!isForbidden)
        return done(null, `${'OK'.green}${refStr}`)
  ...
  *                                                                          */
  page(name, opts) {
    ...
    return function(req, res, done) {      
      ...
      res.render(name || 'index', assign(
        req.locals, {layout,about,js,css,cfg,ud}, {authenticated,session}))

      done(null, `${layout}:${name.dim}`, $req.STOP)
    }
  },
  ...
  *                                                                          */
  notFound(opts) {
    ...
    return function(req, res, done) {
      // $log('mw.notFound'.magenta, onBot, $req.byBot(req, 'notFound'))
      var {ctx} = req
  ...
  error(opts = {}) {
    ...
    return function(e, req, res, next) {
      ...
      // else if (renderCustom)
        ...
        res.render(opts.render.view, assign(opts.render||{}, {error:e,user:req.user}))
      else
      ...
      if (opts.verbose)
        console.log(`err[${e.status||400}].${contentType}`.red, ctx.ud, ctx.uId || ctx.sId, ctx.ref || '', honey.log.request(e, ctx))
      else if (opts.quiet)
