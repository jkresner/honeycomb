module.exports = ({Logic,DAL,$mw,$req}) => ({
  ...
  /**                                                                   cached(
  * Allows utlization of asyncronous cache syncronously later
  ...
  *   Object     .cache instance
  ...
  cached(key, getter, opts) {
    ...
    return function(req, res, done) {
      var cb = (e, r) => {
        $mw.name.call(self, 'data', `cached:${key}`, req) //-- intersting race condition to refactor for
        done(e, e ? null : `${key}`)
      }
  ...
  *              provided defaults to find by '_id' the attribute
  /                                                                         )*/
  param(model, opts) {
    this.noWrap = true
    opts = opts || {}
    ...
    return function(req, res, done, val) {
      ...
      // $log('mw.param', modelName, val, q)
      DAL[modelName].getByQuery(q, (e,r) => {
        if (!r && !e)
  ...
  *   String    .queryKey an attrbute other than '_id' to query on
  ...
  *                                                                         )*/
  recast(model, src, opts) {
    ...
    var required = opts.required || false
    var multi = opts.multi || dest.match(/s$/)
    ...
    return function(req, res, done) {
      ...
      DAL[modelName][getter](query, function(e, r) {        
  ...
  * (2) Enforcing "need to know" access to users with varying privilages
  ...
  * server-side validation seperate to and before any logic runs
  *
  * String     @name usually the Object path of the projectFn
  ...
  *              validation failure or null if valdation passes
  * Object     @opts[optional]
  ...
  *                                                                         )*/
  valid(name, src, validate, opts) {
    ...
    return function(req, res, next) {
      ...
      var error = { message:`${'fail:'.red} `+`${inValid}`.magenta.dim, status: 403 }
  ...
  */
  logic(namespace, fnName, argsMap) {
    ...
    return function(req, res, done) {
      ...
      logic.exec.apply({session,sessionID,user}, args)
    }
  }
