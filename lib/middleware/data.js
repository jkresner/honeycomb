module.exports = ({DAL,$mw,$req}) => ({
  ...
  *                                                                         )*/
  recast(model, src, opts = {}) {
    ...
    let key = opts.queryKey || '_id'
    let dest = opts.dest || model
    let required = opts.required || false
    let merge = opts.merge || false
    let project = opts.project || undefined
    let multi = opts.multi === false ? opts.multi : opts.multi || dest.match(/s$/)
    let getter = multi ? 'getManyById' : 'getByQuery'
    let modelName = model.replace(/s$/,'')
                         .replace(model[0], model[0].toUpperCase())
                         
    if (!DAL || !DAL[modelName] || !DAL[modelName][getter])
      throw Error(`middleware recast.init fail. DAL.${modelName}.${getter} not defined`)

    return function(req, res, done) {
      let val = _.get(req, src),
          query = multi ? val : { [key]:val }
          logData = `${('req.'+src+' =>').dim} ${modelName}.${getter}(${JSON.stringify(query)})`
