module.exports = ({$req},{passport}) => ({
  ...
  *
  ...
  touch(opts) {
    ...
    if (opts.authdData) {
      let attrs         = opts.authdData.split(' ')
      let project       = opts.project || (user => _.pick(user, attrs))
      let serialize     = (user, cb) => cb(null, project(user))
      let deserialize   = (session, cb) => cb(null, session)
  ...
  *   Boolean   .overwite update existing values, defaults to true
  /                                                                         )*/
  remember(src, opts={}) {
    let dest = opts.dest || src.split('.').pop()
    let overwrite = opts.overwrite || true
    this.mwName = `remember:${src}`
    return function(req, res, done) {
