...
const nodeInternal = '_resolveFilename|_modules|_load|native'
const currentDir = process.cwd()
function error(e, maxLines=20) {
  let lns = (e.stack || e).split('\n')
    .filter(ln => !ln.match(nodeInternal) || ln.match('honey'))

  let first = lns.shift()
  return first.red + '\n' + util.format.call(this, _.take(lns, maxLines)
    .map(str => str.replace("  at ", '').trim().replace("  "," ").replace(currentDir,''))
    .map(str => str.replace("(",'').replace(")",'').split(' '))
    .map(words => words.join(' ').replace(words[0], words[0].red).split(':'))
    .map(([info,line]) => `  ${line.gray.dim}\t${info.dim}`)
    .join('\n'))
}
...
function mw(req, mwName) {
  ...
  var u = ((ctx.user||{}).name||'')
  var method = req.method.substr(0,3).cyan.dim
  return `${sId} ${ip} ${UD} ${method}`.cyan.dim+` ${originalUrl.magenta}${ref} ${u.white} `+`${ctx.ua||'ua:null'}`.gray
}


function json(obj, opts = {}) {
  var name = (opts.name||'').yellow
  return name + (obj && Object.keys(obj).length > 0)
    ? ` ${JSON.stringify(obj).replace(/\"([^(\")"]+)\":/g,"$1:".dim).gray}` : ' !Object'
}


module.exports = { strip, error, line, json, time, request, track, issue, mw }
