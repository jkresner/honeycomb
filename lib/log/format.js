...
const nodeInternal = '(coffee-script|mocha)/lib|\\((vm|timers|module|domain).js|createFunctionContext'
// const nodeInternal = '_resolveFilename|_modules|_load|native|timers.js'
const currentDir = process.cwd()

function error(e, maxLines=20) {
  // console.log('nodeInternal'.blue, nodeInternal, 'currentDir'.blue, currentDir)
  let lines = (e.stack||e).split(/\n\s+at/).map(l=>l.trim())
                                           .map(l=>l.replace('/Users/j/src/honey/lib','honey/'))
                                           .filter(l=>l!='')
                                           .map(l=>l.replace('/Users/j/src/','')
                                                    .replace('honey/node_modules/handlebars/dist/cjs/handlebars','handlebars'))

  let first = lines.shift().red+'\n'
  let rest = _.take(                  lines.filter(l => !l.match(nodeInternal))
                    , maxLines-1)
    // console.log('ln', l)
    // console.log('ln', l.match(nodeInternal))
    // return  // || l.match('honey')
...
function line() {
  ...
  let scope = args.shift()
  let verbose = honey.cfg('log.verbose')
  if (colors[scope]||verbose) {
    let theme = colors[scope] ? scope : 'white'
    let namespace = verbose ? scope : scope.split('.')[0]
    let pad = verbose ? 12 : 4
    args[0] = pretty(args[0], 24, theme)
    args.unshift(pretty(namespace, pad, theme).dim)
...
function time(label, msg) {
  ...
  let verbose = honey.cfg('log.verbose')
  let namespace = verbose ? 'app.init' : label
  let pad = verbose ? 13 : 5
  if (!start) timed.set(label, now)

  stamps.set(label, now)

  return util.format.apply(this, ['%s%s%s%s',
    pretty(namespace, pad, 'app.init'),
