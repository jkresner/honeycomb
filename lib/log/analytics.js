...
var Doc = (DAL, {app, map, toUserId, formatter, noop}) =>
  function(type, ctx, d, cb) {
    ...
    if (d.ctx) {
      if (d.ctx.hasOwnProperty('prefix')) d.ctx.prefix = honey.log.strip(d.ctx.prefix)
...
function aliasDocs(DA, sId, uId, cb) {
  if (!DA) return
  // $log('aliasDocs'.yellow, {sId,uId:{$exists:0}})
  DA.getManyByQuery({sId,uId:{$exists:0}}, {select:'_id'}, (e, ups) => {
    if (e || ups.length == 0) return cb(e, null)
    DA.updateSetBulk(ups.map(up => assign(up,{uId}), cb))
  })
}
...
class Analytics {
  ...
  constructor(cfg, model, opts={}) {
    ...
    let createDoc = Doc(DAL, opts)
    let {event,impression,issue,view,campaign} = cfg.model.collections
    ...
    if (event) this.event = function(ctx, name, data, cb) {
      ...
      if (alias) {
        ctx.user = alias
        let uId = opts.toUserId(ctx.user._id)
        aliasDocs(DAL[event], ctx.sId, uId, opts.noop)
