...
class MongoseDA {
  ...
  constructor(modelName, dir, daOpts) {
    ...
    this._updateOne = function(set, _id, fields, opts, cb) {
      ...
      var q = Query.byId(_id)
      LOG('modl.write',`${modelName}.updateOne[${_id}]`, q, update, opts)

      if (set)
        model.findOneAndUpdate(q, update, opts, (e,r) => {
          if (e || !r) $log(`${modelName}.updateOne.error`.red, q, `${(e||{}).message||'no update'}`.red, JSON.stringify(update||{}).yellow)
          if (r) r = r.toObject()
          if (cb) cb(e, r)
        })
      else
        collection.findAndModify(q, [['_id',1]], update, opts, (e,r) => {
          if (e || !r) $log(`${modelName}.updateOne.error`.red, q, `${(e||{}).message}||'no update'`.red, JSON.stringify(update||{}).yellow)
          if (cb) cb(e, r.value)
        })
    }
