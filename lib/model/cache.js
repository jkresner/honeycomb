...
cache.flush = function(pattern)
{
  for (var key in cache) {
    ...
    if (key.match(pattern)) {
      delete cache[key]
      LOG('modl.cache', `cache:flush ${item}`)
    }
  }
}
...
cache.get = function(key, getForCacheFn, cb) {
  ...
  // console.log('key', key, getForCacheFn)
  getForCacheFn( (e,r) => {
    if (e) return cb(e)
    cache[key] = r
    LOG('modl.cache', `cache${r&&r.length!=null?'['+r.length+']':' {}'} SET${key}`)
    cb(null, r)
  })
}
...
cache.require = function(getters, cb) {
  var prom = (key, getter) => new Promise(function(resolve, reject) {
    ...
    getter((e, r) => {
      ...
      cache[key] = r
      LOG('modl.cache', `cache${r&&r.length!=null?'['+r.length+']':' {}'} REQUIRED:${key}`)
      resolve(key)
    })
  })
