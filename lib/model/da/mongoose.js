...
class MongoseDA {
  ...
  constructor(modelName, dir, daOpts) {
    ...
    this._find = function(many, conditions, opts, cb) {
      ...
      LOG('modl.read', `${modelName}.${op}`, `${stringify(conditions)} ${stringify(assign(options,select?{select}:{}))}`.gray)
    ...
    this._updateOne = function(op, _id, fields, opts, cb) {
      ...
      LOG('modl.write',`${modelName}.updateOne.${op}`, `${_id} ${stringify(up)}, ${stringify(opts)}`.gray)

      if (op == '$set')
        model.findOneAndUpdate(q, up, opts, (e,r) => {
          if (!e&&!r) e = Error(`MongooseDA null update.${op}`)
          if (e) $log(`${modelName}.updateOne[${_id}].${op}.e`.red, e.message, stringify(up||{}).dim)
          if (cb) cb(e, r ? r.toObject() : r)
        })
      else
        collection.findOneAndUpdate(q, up, {returnOriginal:false}, (e,r) => {
          if (!e&&!r) e = Error(`MongooseDA null update.${op}`)
          if (e) $log(`${modelName}.updateOne[${_id}].${op}.e`.red, e.message, stringify(up||{}).dim)
    ...
    this._create = function(o, cb) {
      LOG('modl.write', `${modelName}.create`, stringify(o).gray)
       new model( o ).save( (e,r) => {
        if (e) $log(`modl.${modelName}.create.error`.red, `${(e||{}).message}`.red, stringify(o||{}).yellow)
        if (r) r = r.toObject()
        if (cb) cb(e, r)
      })
    }
