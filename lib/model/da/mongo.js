...
class MongoDA {
  ...
  constructor(schemaName, dir, daOpts) {
    ...
    this._find = function(many, conditions, opts, cb) {
      ...
      var options = { lean: true, projection: select }
      if (many) {
        if (sort) assign(options,{sort})
        if (limit) assign(options,{limit})
        LOG('modl.read', `${modelName}.find`, `${stringify(conditions)} ${stringify(options)}`.gray)
        collection.find(conditions, options).toArray((e, r) => {
